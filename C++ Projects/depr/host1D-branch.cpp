...
class propagator
{
	...
	std::vector<thrust::device_vector<double>> outVector;
	...
	thrust::device_vector<double> sums = thrust::host_vector<double>(8, 0.0);
...
public:
	propagator(const int dim, thrust::host_vector<double> ellipseArray)
	{
		...
		sumIter = thrust::make_zip_iterator(thrust::make_tuple(sums.begin(), sums.begin()+1, sums.begin()+2, sums.begin()+3,sums.begin()+4, sums.begin()+5, sums.begin()+6, sums.begin()+7));
	}
	...
	void propagate()
	{
		...
		thrust::device_vector<double>::iterator start;
		thrust::device_vector<double>::iterator end;// = std::next(start, steps);
		...
		for(int j = 1; j < width -1; j++)
			for (int i = 1; i < width-1; i++) // for each node..
			{
				...
				if (thrust::equal(start, end, initArrayD.begin()))
					continue;
