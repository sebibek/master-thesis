...
void computeGlyphs(std::vector<std::vector<double>>& glyphBuffer, std::vector<std::vector<bool>>& signMap, std::vector<std::vector<double>>& glyphParameters)
{
	...
	for (int i = 0; i < matrixList.size(); i++)
	{
		...
		double deg1 = atan2(y1, x1) * 180.0 / M_PI; // use vector atan2 to get rotational angle (phase) of both basis vectors in [-180\B0,180\B0]
		double deg2 = atan2(y2, x2) * 180.0 / M_PI; // use vector atan2 to get rotational angle (phase) of both basis vectors [-180\B0,180\B0]
		...
		// shift (normalize) degs from [-180\B0,180\B0] into the interval [0\B0,360\B0] - "circular value permutation"
