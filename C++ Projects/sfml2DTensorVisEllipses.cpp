...
class propagator
{
	...
	std::string cosLobeX = "cos(theta)"; // cosine lobe in +x-direction (in polar plot)
	std::string cosLobeY = "cos(theta-pi/2)"; // cosine lobe in -y-direction (in polar plot) UP!!!
	std::string cosLobeYneg = "cos(theta+pi/2)"; // cosine lobe in +y-direction (in polar plot) // lobe in positive y-direction DOWN!
	std::string cosLobeXneg = "cos(theta+pi)"; // cosine lobe in -x-direction (in polar plot)
	...
	// define central lobes and directions vectors (pre-computed)
	std::vector<string> centralLobesNeg{ cosLobeYneg , cosLobeXneg, cosLobeXneg, cosLobeYneg, cosLobeXneg, cosLobeYneg };
	std::vector<double>	centralDirectionsNeg{ 3.0 / 2 * M_PI + 1.0172232666228471, M_PI / 2 + 0.5535748055013016,  M_PI, M_PI + 0.5535748055013016, M_PI + 1.0172232666228471, 3.0 / 2 * M_PI };	
	std::vector<string> centralLobesPos{ cosLobeX, cosLobeY, cosLobeX, cosLobeY, cosLobeY, cosLobeX };
	std::vector<double>	centralDirectionsPos{ 0.0, 0.5535748055013016, 1.0172232666228471, M_PI / 2, M_PI / 2 + 1.0172232666228471, 3.0 / 2 * M_PI + 0.5535748055013016 };

	// create indices Maps
	std::vector<int> indicesPos{ 0, 0, 1, 1, 2, 3 };
	std::vector<int> indicesLobesPos{ 0, 1, 0, 1, 1, 0 }; // indicate central lobe directions
	std::vector<int> indicesNeg{ 0, 1, 2, 2, 3, 3 };
	std::vector<int> indicesLobesNeg{ 1, 0, 0, 1, 0, 1 }; // indicate central lobe directions
	...
	std::vector<double> apertureAnglesPos{ alpha, beta, beta, alpha, beta, beta };
	std::vector<double> apertureAnglesNeg{ beta, beta, alpha, beta, beta, alpha };
	...
	std::vector<double> anPos;
	std::vector<double> bnPos;// (21, 0.0);
	std::vector<double> anNeg;
	std::vector<double> bnNeg;// (21, 0.0);
	
	// create process(ed) map for cells already processed
	std::vector<bool> processMap; // create a binary process(ed) map

public:
	propagator(int dim): processMap(dim, false), anPos(21,0.0), bnPos(21,0.0), anNeg(21, 0.0), bnNeg(21, 0.0) // constructor propagator which calls std::vector constructors for processMap, an, bn
	{
		parser.DefineConst("pi", pi);
		parser.DefineVar("theta", &t);
	}

	void propagate(int jIndex, int iIndex, int length, std::vector<std::string>& functionStrCosPos, std::vector<std::string>& functionStrCosNeg, std::vector<std::string>& functionStrSinPos, std::vector<std::string>& functionStrSinNeg, std::vector<std::vector<std::vector<double>>>& coefficientArrayPos, std::vector<std::vector<std::vector<double>>>& coefficientArrayNeg)
	{
		// create neighborhood, for tagging non-existent neighbors..
		neighborhood hood(jIndex, iIndex, length, processMap);

		// query coefficient vectors for evaluating current light profile... get from current position (jIndex,iIndex)
		anPos = coefficientArrayPos.at(iIndex + jIndex * length).front(); // ..cosine coefficient vector
		bnPos = coefficientArrayPos.at(iIndex + jIndex * length).back(); // ..sine coefficient vector
		anNeg = coefficientArrayNeg.at(iIndex + jIndex * length).front(); // ..cosine coefficient vector
		bnNeg = coefficientArrayNeg.at(iIndex + jIndex * length).back(); // ..sine coefficient vector
		//bnNeg = coefficientArrayNeg.at(iIndex + jIndex * length).back(); // ..sine coefficient vector

		// create fString w. a0 (offset)
		std::string fStringPos = std::to_string(anPos.front());
		std::string fStringNeg = std::to_string(anNeg.front());
		std::string a0Pos = std::to_string(anPos.front());
		std::string a0Neg = std::to_string(anNeg.front());
		// create fString for cosine and sine series
		std::string fStringCosPos = "0.0";
		std::string fStringSinPos = "0.0";
		std::string fStringCosNeg = "0.0";
		std::string fStringSinNeg = "0.0";

		// POSITIVE
		// set up Fourier Series expansion for Pos vectors...
		//for (int i = 1; i <= anPos.size(); i++)
		//{
		//	if (i > 1 && (anPos.at(i) < thresh && anPos.at(i - 1) < thresh && bnPos.at(i) < thresh && bnPos.at(i - 1) < thresh)) // if threshold exceeded..
		//		break; // abort Fourier series expansion to obtain shorter functionStrings (approximations)
		//	fStringPos += "+" + std::to_string(anPos.at(i)) + "*cos(" + std::to_string(i) + "*theta)" + "+" + std::to_string(bnPos.at(i)) + "*sin(" + std::to_string(i) + "*theta)";
		//}

		// set up Cosine Series expansion for Pos vectors...
		for (int i = 1; i <= anNeg.size(); i++)
		{
			if (i > 1 && (anNeg.at(i) < thresh && anNeg.at(i - 1) < thresh && bnNeg.at(i) < thresh && bnNeg.at(i - 1) < thresh)) // if threshold exceeded..
				break; // abort Fourier series expansion to obtain shorter functionStrings (approximations)
			fStringCosPos += "+(" + std::to_string(anPos.at(i)) + ")*cos(" + std::to_string(i) + "*theta)";
		}

		// set up Sine Series expansion for Pos vectors...
		for (int i = 1; i <= anNeg.size(); i++)
		{
			if (i > 1 && (anNeg.at(i) < thresh && anNeg.at(i - 1) < thresh && bnNeg.at(i) < thresh && bnNeg.at(i - 1) < thresh)) // if threshold exceeded..
				break; // abort Fourier series expansion to obtain shorter functionStrings (approximations)
			fStringSinPos += "+(" + std::to_string(bnPos.at(i)) + ")*sin(" + std::to_string(i) + "*theta)";
		}

		// NEGATIVE
		// set up Fourier Series expansion for Neg vectors...
		//for (int i = 1; i <= anNeg.size(); i++)
		//{
		//	if (i > 1 && (anNeg.at(i) < thresh && anNeg.at(i - 1) < thresh && bnNeg.at(i) < thresh && bnNeg.at(i - 1) < thresh)) // if threshold exceeded..
		//		break; // abort Fourier series expansion to obtain shorter functionStrings (approximations)
		//	fStringNeg += "+(" + std::to_string(anNeg.at(i)) + ")*cos(" + std::to_string(i) + "*theta)" + "+(" + std::to_string(bnNeg.at(i)) + ")*sin(" + std::to_string(i) + "*theta)";
		//}

		// set up Cosine Series expansion for Neg vectors...
		for (int i = 1; i <= anNeg.size(); i++)
		{
			if (i > 1 && (anNeg.at(i) < thresh && anNeg.at(i - 1) < thresh && bnNeg.at(i) < thresh && bnNeg.at(i - 1) < thresh)) // if threshold exceeded..
				break; // abort Fourier series expansion to obtain shorter functionStrings (approximations)
			fStringCosNeg += "+(" + std::to_string(anNeg.at(i)) + ")*cos(" + std::to_string(i) + "*theta)";
		}

		// set up Sine Series expansion for Neg vectors...
		for (int i = 1; i <= anNeg.size(); i++)
		{
			if (i > 1 && (anNeg.at(i) < thresh && anNeg.at(i - 1) < thresh && bnNeg.at(i) < thresh && bnNeg.at(i - 1) < thresh)) // if threshold exceeded..
				break; // abort Fourier series expansion to obtain shorter functionStrings (approximations)
			fStringSinNeg += "+(" + std::to_string(bnNeg.at(i)) + ")*sin(" + std::to_string(i) + "*theta)";
		}


		// update current functionStr w. fStringPos/fStringNeg to maintain the polar functions in gríd
		functionStrCosPos.at(iIndex + jIndex * length) = fStringCosPos;
		functionStrCosNeg.at(iIndex + jIndex * length) = fStringCosNeg;
		functionStrSinPos.at(iIndex + jIndex * length) = fStringSinPos;
		functionStrSinNeg.at(iIndex + jIndex * length) = fStringSinNeg;

		// POSITIVE COEFFICIENTS //

		// iterate through central directions array to distribute (spread) energy (intensity) to the cell neighbors
		for (int k = 0; k < centralDirectionsPos.size(); k++) // k - (Strahl-)keulenindex
		{
			int nIndex = indicesPos.at(k); // create index to capture the 4 neighbors (in 2D)
			...
			t = centralDirectionsPos.at(k); // set theta variable to current central direction
			double intensityPos = 0.0; // luminous intensity obtained from stringFunction (muParser..)
			double intensityNeg = 0.0;

			if (apertureAnglesPos.at(k) == alpha) // special case, if totally oriented with aperture angle alpha...
			{
				if (indicesLobesPos.at(k) == 0) // if oriented horizontally..
					parser.SetExpr(fStringCosPos); // evaluate only positive cosines!!!
				else // oriented vertically
					parser.SetExpr(fStringSinPos); // evaluate only positive sines!!!
			}
			else
			{
				if (k == centralDirectionsPos.size() - 2) // special case 2nd last
				{
					parser.SetExpr(fStringCosNeg); // swap setExpression temporary to evaluate negative fourier coefficients as well..
					intensityNeg = parser.Eval();
					parser.SetExpr(fStringSinPos); // .. 
				}
				else if (k == centralDirectionsPos.size() - 1) // special case last
				{
					parser.SetExpr(fStringSinNeg); // swap setExpression temporary to evaluate negative fourier coefficients as well..
					intensityNeg = parser.Eval();
					parser.SetExpr(fStringCosPos); // .. 
				}
				else // default case, reutilize intensityNeg to evaluate both series seperately and do the superposition absolutely (geometrical)
				{
					parser.SetExpr(fStringSinPos);
					intensityNeg = parser.Eval();
					parser.SetExpr(fStringCosPos);
			...
			intensityPos = anPos.front() + parser.Eval();
			
			// swap back
			double area = (abs(intensityPos)+abs(intensityNeg)) * apertureAnglesPos.at(k); // integrate over angle in polar coordinates (pi*r^2) to obtain total luminous flux (power) received by adjacent cell faces
			...
			functionString = std::to_string(mag) + "*" + centralLobesPos.at(k); // strFunction --> functionString

			switch (nIndex) // switch nIndex to cope with changing neighbor locations..., accumulate fourier coefficients in coefficientArray
			{
			case 0: coefficientArrayPos.at(iIndex + 1 + jIndex * length) = coefficientArrayPos.at(iIndex + 1 + jIndex * length) + calcCoeff(strFunction); break; // right neighbor
			case 1: coefficientArrayPos.at(iIndex + (jIndex - 1) * length) = coefficientArrayPos.at(iIndex + (jIndex - 1) * length) + calcCoeff(strFunction); break; // top neighbor
			case 2: coefficientArrayPos.at(iIndex - 1 + jIndex * length) = coefficientArrayPos.at(iIndex - 1 + jIndex * length) + calcCoeff(strFunction); break;// left neighbor
			case 3: coefficientArrayPos.at(iIndex + (jIndex + 1) * length) = coefficientArrayPos.at(iIndex + (jIndex + 1) * length) + calcCoeff(strFunction); break;// bottom neighbor
			}
		}
		...
		// iterate through central directions array to distribute (spread) energy (intensity) to the cell neighbors
		for (int k = 0; k < centralDirectionsNeg.size(); k++) // k - (Strahl-)keulenindex
		{
			int nIndex = indicesNeg.at(k); // create index to capture the 4 neighbors (in 2D)
			...
			t = centralDirectionsNeg.at(k); // set theta variable to current central direction
			double intensityPos = 0.0; // luminous intensity obtained from stringFunction (muParser..)
			double intensityNeg = 0.0;

			if (apertureAnglesNeg.at(k) == alpha)
			{
				if (indicesLobesNeg.at(k) == 0)
					parser.SetExpr(fStringCosNeg);
				else
					parser.SetExpr(fStringSinNeg);
			}
			else
			{
				if (k == 0)
				{
					parser.SetExpr(fStringCosPos); // swap setExpression temporary to evaluate negative fourier coefficients as well..
					intensityPos = parser.Eval();
					parser.SetExpr(fStringSinNeg); // .. 
				}
				else if (k == 1)
				{
					parser.SetExpr(fStringSinPos); // swap setExpression temporary to evaluate negative fourier coefficients as well..
					intensityPos = parser.Eval();
					parser.SetExpr(fStringCosNeg); // .. 
				}
				else
			...
			intensityNeg = anNeg.front() + parser.Eval();

			// swap back
			double area = (abs(intensityPos) + abs(intensityNeg)) * apertureAnglesNeg.at(k); // integrate over angle in polar coordinates (pi*r^2) to obtain total luminous flux (power) received by adjacent cell faces
			double mag = area / 2.0; // since int(cos(w)) (w. negative values clamped to zero) yields = 2 --> normalize to energy 1 (spread energy area over the cosine)

			// ReProjection of Flow Lobes //

			// create oriented function (string) from centralLobes vector (corresponding to centralDirections and apertureAngles)
			functionString = std::to_string(mag) + "*" + centralLobesNeg.at(k); // strFunction --> functionString

			switch (nIndex) // switch nIndex to cope with changing neighbor locations..., accumulate fourier coefficients in coefficientArray
			{
			case 0: coefficientArrayNeg.at(iIndex + 1 + jIndex * length) = coefficientArrayNeg.at(iIndex + 1 + jIndex * length) + calcCoeff(strFunction); break; // right neighbor
			case 1: coefficientArrayNeg.at(iIndex + (jIndex - 1) * length) = coefficientArrayNeg.at(iIndex + (jIndex - 1) * length) + calcCoeff(strFunction); break; // top neighbor
			case 2: coefficientArrayNeg.at(iIndex - 1 + jIndex * length) = coefficientArrayNeg.at(iIndex - 1 + jIndex * length) + calcCoeff(strFunction); break;// left neighbor
			case 3: coefficientArrayNeg.at(iIndex + (jIndex + 1) * length) = coefficientArrayNeg.at(iIndex + (jIndex + 1) * length) + calcCoeff(strFunction); break;// bottom neighbor
			}
		}
