...
{ 
	...
	thrust::host_vector<DoubleIterator>* startVec;
	thrust::host_vector<DoubleIterator>* readGlyphStartVec;
	thrust::host_vector<Double8Iterator>* cellDestinations;
	thrust::host_vector<Double8Iterator>* firstReadGlyphVec;

	thrust::device_vector<double>* initArray;

	std::vector<thrust::host_vector<double>>* sums;
	std::vector<std::vector<thrust::device_vector<double>>>* outVector;



public:
    propagateCell(int _width, int _steps, double _radres, Double8Iterator* _firstWeights, Double8Iterator* _firstCosine, Double8Iterator* _lastCosine, thrust::host_vector<DoubleIterator>* _startVec, thrust::host_vector<DoubleIterator>* _readGlyphStartVec, thrust::host_vector<Double8Iterator>* _cellDestinations, thrust::host_vector<Double8Iterator>* _firstReadGlyphVec, std::vector<thrust::host_vector<double>>* _sums, std::vector<std::vector<thrust::device_vector<double>>>* _outVector, thrust::device_vector<double>* _initArray)
    {
		width = _width;
		steps = _steps;
		radres = _radres;
		...
		startVec = _startVec;
		readGlyphStartVec = _readGlyphStartVec;
		cellDestinations = _cellDestinations;
		firstReadGlyphVec = _firstReadGlyphVec;
   ...
   template <typename Tuple>
    ...
    void operator()(Tuple& t)
    {
		...
		firstOut = thrust::make_zip_iterator(thrust::make_tuple(outVector[0][index][0].begin(), outVector[0][index][1].begin(), outVector[0][index][2].begin(), outVector[0][index][3].begin(), outVector[0][index][4].begin(), outVector[0][index][5].begin(), outVector[0][index][6].begin(), outVector[0][index][7].begin()));// = thrust::get<8>(t);
		lastOut = thrust::make_zip_iterator(thrust::make_tuple(outVector[0][index][0].end(), outVector[0][index][1].end(), outVector[0][index][2].end(), outVector[0][index][3].end(), outVector[0][index][4].end(), outVector[0][index][5].end(), outVector[0][index][6].end(), outVector[0][index][7].end()));// = thrust::get<8>(t);		//lastOut = thrust::make_zip_iterator(thrust::make_tuple(outVector[0].end(), outVector[1].end(), outVector[2].end(), outVector[3].end(), outVector[4].end(), outVector[5].end(), outVector[6].end(), outVector[7].end())); //= thrust::get<9>(t);

		readGlyphStart = readGlyphStartVec[0][index];//thrust::get<3>(t);
		...
		firstReadGlyph = firstReadGlyphVec[0][index];//thrust::make_zip_iterator(thrust::make_tuple(readGlyphStart, readGlyphStart, readGlyphStart, readGlyphStart, readGlyphStart, readGlyphStart, readGlyphStart, readGlyphStart));
		...
		/*sums[0][index][0] = thrust::reduce(&thrust::get<0>(firstOut[0]), &thrust::get<0>(lastOut[0]))*radres;
		sums[0][index][1] = thrust::reduce(&thrust::get<1>(firstOut[0]), &thrust::get<1>(lastOut[0]))*radres;
		sums[0][index][2] = thrust::reduce(&thrust::get<2>(firstOut[0]), &thrust::get<2>(lastOut[0]))*radres;
		sums[0][index][3] = thrust::reduce(&thrust::get<3>(firstOut[0]), &thrust::get<3>(lastOut[0]))*radres;
		sums[0][index][4] = thrust::reduce(&thrust::get<4>(firstOut[0]), &thrust::get<4>(lastOut[0]))*radres;
		sums[0][index][5] = thrust::reduce(&thrust::get<5>(firstOut[0]), &thrust::get<5>(lastOut[0]))*radres;
		sums[0][index][6] = thrust::reduce(&thrust::get<6>(firstOut[0]), &thrust::get<6>(lastOut[0]))*radres;
		sums[0][index][7] = thrust::reduce(&thrust::get<7>(firstOut[0]), &thrust::get<7>(lastOut[0]))*radres;*/
		...
		sumIter = thrust::make_zip_iterator(thrust::make_tuple(thrust::make_constant_iterator(sums[0][index][0]), thrust::make_constant_iterator(sums[0][index][1]), thrust::make_constant_iterator(sums[0][index][2]), thrust::make_constant_iterator(sums[0][index][3]), thrust::make_constant_iterator(sums[0][index][4]), thrust::make_constant_iterator(sums[0][index][5]), thrust::make_constant_iterator(sums[0][index][6]), thrust::make_constant_iterator(sums[0][index][7])));//thrust::make_zip_iterator(thrust::make_tuple(thrust::make_constant_iterator(sums.begin()), thrust::make_constant_iterator(sums.begin() + 1), thrust::make_constant_iterator(sums.begin() + 2), thrust::make_constant_iterator(sums.begin() + 3), thrust::make_constant_iterator(sums.begin() + 4), thrust::make_constant_iterator(sums.begin() + 5), thrust::make_constant_iterator(sums.begin() + 6), thrust::make_constant_iterator(sums.begin() + 7)));
		
		// get dst zip iterator
		firstDst = cellDestinations[0][index]; //thrust::make_zip_iterator(thrust::make_tuple(dst0, dst1, dst2, dst3, dst4, dst5, dst6, dst7)); // TODO: prepare firstDst as host::vector for every cell index, ALSO prepare start,end, readGlyph
...
class propagator
{
	...
	thrust::device_vector<double> sampleBufferDst;

	thrust::host_vector<double> sampleBufferHost;
	...
	thrust::device_vector<double>::iterator glyphStart;
	thrust::device_vector<double>::iterator glyphEnd;
	thrust::device_vector<double>::iterator readGlyphStart;
	thrust::device_vector<double>::iterator readGlyphEnd;
	thrust::device_vector<double>::iterator dstStart;*/
	thrust::device_vector<double>::iterator glyphStart;
	thrust::device_vector<double>::iterator glyphEnd;
	...
	thrust::device_vector<double> sampleBuffer0;
	thrust::device_vector<double> sampleBuffer1;
	thrust::device_vector<double> sampleBuffer2;
	thrust::device_vector<double> sampleBuffer3;
	thrust::device_vector<double> sampleBuffer4;
	thrust::device_vector<double> sampleBuffer5;
	thrust::device_vector<double> sampleBuffer6;
	thrust::device_vector<double> sampleBuffer7;
...
public:
	...
	propagator(const int dim, thrust::host_vector<double>& ellipseArray)
	{
		...
		tMeans = thrust::device_vector<double>(dim, 0.0);
		sums = std::vector<thrust::host_vector<double>>(dim, thrust::host_vector<double>(8, 0.0));
		outVector = std::vector<std::vector<thrust::device_vector<double>>>(dim, std::vector<thrust::device_vector<double>>(8, initArray));
		...
		firstReadGlyphVec = thrust::host_vector<Double8Iterator>(dim);
		startVec = thrust::host_vector<DoubleIterator>(dim);
		readGlyphStartVec = thrust::host_vector<DoubleIterator>(dim);
		...
		for(int j = 1; j < width -1; j++)
			for (int i = 1; i < width-1; i++) // for each node..
			{
				...
				// define iterators for accessing current sample
				startVec[index] = std::next(sampleBufferA.begin(), index * steps);

				readGlyphStartVec[index] = std::next(readGlyph.begin(), index * steps);

				firstReadGlyphVec[index] = thrust::make_zip_iterator(thrust::make_tuple(readGlyphStartVec[index], readGlyphStartVec[index], readGlyphStartVec[index], readGlyphStartVec[index], readGlyphStartVec[index], readGlyphStartVec[index], readGlyphStartVec[index], readGlyphStartVec[index]));
			}
	...
	void propagate()
	{
		...
		// 1 propagation cycle
		thrust::for_each(zipFirst, zipLast, propagateCell(width, steps, radres, &firstWeights, &firstCosine, &lastCosine, &startVec, &readGlyphStartVec, &cellDestinations, &firstReadGlyphVec, &sums, &outVector, &initArrayD));

		// add up 8 individual sample buffers
		thrust::transform(destinationsFirst, destinationsLast, sampleBufferDst.begin(), elementAcc());
		cout << "HERE 4" << endl;

		//thrust::copy(sampleBufferB.begin(), sampleBufferB.end(), sampleBufferHost.begin()); // WORKS
		...
		cout << "HERE 5" << endl;
		//sampleBufferHost = sampleBufferB;
		for(int i = 0; i < dim; i++)
			meanA += sampleBufferDst[i];
		//meanA = thrust::reduce(sampleBufferHost.begin(), sampleBufferHost.end(), 0.0)*radres;
		cout << "HERE 6" << endl;
	...
	thrust::host_vector<double> propagateDist(int i, int j, int t)
	{
		...
		sampleBufferHost = sampleBufferInit;
		sampleBufferHost[index] = steps;
		sampleBufferA = sampleBufferHost;
		int ctr = 0;
