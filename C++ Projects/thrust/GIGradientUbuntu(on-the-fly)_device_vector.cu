...
void computeGlyphs(std::vector<thrust::device_vector<double>>& glyphBuffer, std::vector<std::vector<bool>>& signMap, std::vector<std::vector<double>>& glyphParameters)
{
	...
	for (int i = 0; i < matrixList.size(); i++)
	{
		...
		{
			glyphBuffer.at(i)[round(deg1*steps / 360)] = sv1;
			glyphBuffer.at(i)[static_cast<int>(round(deg1*steps / 360 + steps / 2)) % steps] = sv1;
			sum += 2 * sv1;
		}
		else
		{
...
int main(int argc, char* argv[])
{
	...
	std::vector<thrust::device_vector<double>> sampleBufferA(dim);
	std::vector<thrust::device_vector<double>> sampleBufferB(dim);
	std::vector<thrust::device_vector<double>> sampleBufferInit(dim);
	std::vector<thrust::device_vector<double>> glyphBuffer(dim);
