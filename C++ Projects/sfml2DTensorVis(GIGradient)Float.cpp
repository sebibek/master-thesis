...
float intensity = 2.1; // --> initial intensity val
float thresh = 0.001;
...
void computeGlyphs(std::vector<std::vector<float>>& glyphBuffer, std::vector<std::vector<bool>>& signMap, std::vector<std::vector<float>>& glyphParameters)
{
	...
	// define parameters
	float radres = (2 * pi) / steps;

	std::complex<float> sigma1(0, 0);
	std::complex<float> sigma2(0, 0);
	...
	{
		float y1 = svdList.at(i).matrixU().col(0)[1]; // use x - coordinate of both semi-axes -- Get LEFT U-vector
		float x1 = svdList.at(i).matrixU().col(0)[0]; // use x - coordinate of both semi-axes
		float y2 = svdList.at(i).matrixU().col(1)[1]; // use x - coordinate of both semi-axes -- Get RIGHT U-vector
		float x2 = svdList.at(i).matrixU().col(1)[0]; // use x - coordinate of both semi-axes
		float xx = matrixList.at(i).row(0)[0]; // "sigma_xx"
		float xy = matrixList.at(i).row(0)[1]; // "sigma_xy"
		float yx = matrixList.at(i).row(1)[1]; // "sigma_yx"
		float yy = matrixList.at(i).row(1)[1]; // "sigma_yy"
		float deg1 = atan2(y1, x1) * 180.0 / M_PI; // use vector atan2 to get rotational angle (phase) of both basis vectors in [-180°,180°]
		float deg2 = atan2(y2, x2) * 180.0 / M_PI; // use vector atan2 to get rotational angle (phase) of both basis vectors [-180°,180°]
		...
		sigma1 = std::complex<float>(0.5*(xx + yy), 0) + float(0.5)*sqrt(std::complex<float>((xx - yy)*(xx - yy) + 4 * xy*yx, 0));
		sigma2 = std::complex<float>(0.5*(xx + yy), 0) - float(0.5)*sqrt(std::complex<float>((xx - yy)*(xx - yy) + 4 * xy*yx, 0));
		...
		float sv1 = svdList.at(i).singularValues()[0];
		float sv2 = svdList.at(i).singularValues()[1];
		float dot = sv2 * sv1;
		...
		else
		{
			...
			{
				float val = dot / sqrt(sv2*sv2*cos(j*radres - deg1 * (M_PI / 180.0))*cos(j*radres - deg1 * (M_PI / 180.0)) + sv1 * sv1*sin(j*radres - deg1 * (M_PI / 180.0))*sin(j*radres - deg1 * (M_PI / 180.0))); //--> ellipse equation, evaluate for tMean (sum2)
				sum += val;
				glyphBuffer.at(i).at(j) = val;
			}
		}
		float rMean = sum / steps; // compute rMean from cartesian (rectangular) energy-based integral as opposed to the polar integral relevant to the geometrical (triangular/circular) area
		...
		std::transform(glyphBuffer.at(i).begin(), glyphBuffer.at(i).end(), glyphBuffer.at(i).begin(), std::bind(std::multiplies<float>(), std::placeholders::_1, 1.0/rMean));
		//glyphBuffer.at(i) = 1.0 / rMean * glyphBuffer.at(i);
	}
}

std::vector<float> sample(float(*f)(float x), std::vector<std::vector<float>>& sampleArray, int jIndex, int iIndex)
{
	std::vector<float> sample(steps, 0.0);
...
class propagator
{
	...
	float alpha = 36.8699 * M_PI / 180;
	float beta = 26.5651 * M_PI / 180;
	...
	std::vector<std::vector<float>>* sampleBufferA;
	std::vector<std::vector<float>>* sampleBufferB;
	std::vector<std::vector<float>>* glyphBuffer;
	
	std::vector<std::vector<float>> cosines;
	
	// create sample vector (dynamic)
	std::vector<float> read;
	std::vector<float> glyph;
	std::vector<float> out;

	neighborhood hood; 
	bool flag = false;

	std::vector<float> initArray;
	std::vector<int> deltaIndex{ 1, 1 - width, -width, -1 - width, -1, -1 + width, width, 1 + width };
public:
	propagator(const int dim, float* mean, std::vector<std::vector<float>>* sampleBuffA, std::vector<std::vector<float>>* sampleBuffB, std::vector<std::vector<float>>* ellipseArray)
	{
		...
		read = std::vector<float>(steps, 0.0);
		glyph = std::vector<float>(steps, 0.0);
		out = std::vector<float>(steps, 0.0);
		initArray = std::vector<float>(steps, 0.0);
		...
		for (int k = 0; k < 8; k++) // for each node..
		{
			...
			std::vector<float> cosK(steps, 0.0);
			for (int j = midIndex - shiftIndex; j <= midIndex + shiftIndex; j++) // for each step (along edge)..
			{
				...
				float res = clip(cos((j_index - midIndex) * radres), float(0.0), float(1.0));
		...
		for (int k = 0; k < 8; k++) // for each node..
			cosines.at(k) = float(1.0) / cosine_sum * cosines.at(k);
	...
	void propagate()
	{
		...
		for (int i = 0; i < width*height; i++) // for each node..
		{
			...
			float iMean = sum1 / steps; // -->tinc(dt) is a constant that can be drawn out of the integral
			...
			float tMean = sum2 / steps; // -->tinc(dt) is a constant that can be drawn out of the integral
			...
			float tiMean = sum3 / steps; // -->tinc(dt) is a constant that can be drawn out of the integral
			...
			for (int k = 0; k < 8; k++) // for each adjacent edge...
			{
				...
				for (int j = midIndex - index; j <= midIndex + index; j++) // for each step (along edge)..
				{
					...
					float val = cFactor*read.at(j_index)*glyph.at(j_index);
					...
					//	float res = val * clip(cos((j_index - l_index) * radres), 0.0, 1.0);// *clip(cos(round(offset / radres) - dirIndex * pi / 2), 0.0, 1.0); // integrate over angle in polar
				...
				// multiply respective cosine cone by valsum*=radres, because of energy normalization to cosine_sum (pre-computed in constructor)
				std::transform(out.begin(), out.end(), out.begin(), std::bind(std::multiplies<float>(), std::placeholders::_1, val_sum *= radres));
				...
				*meanA += val_sum;
				...
				std::transform(sampleBufferB->at(index).begin(), sampleBufferB->at(index).end(), out.begin(), sampleBufferB->at(index).begin(), std::plus<float>());
			}
...
//template <typename T>
float acc(std::vector<float>& vec)
{
	float sum = 0.0;
...
//template <typename T>
float acc2(std::vector<std::vector<float>>& vec)
{
	float sum = 0.0;
	for (int i = 0; i < vec.size(); i++)
		sum += acc(vec.at(i));
...
int main(int argc, char* argv[])
{
	...
	const std::vector<float> initArray(steps, 0.0);

	// define dual buffers for propagation
	std::vector<std::vector<float>> sampleBufferA((width*height), initArray);
	std::vector<std::vector<float>> sampleBufferB((width*height), initArray);
	std::vector<std::vector<float>> sampleBufferMem((width*height), initArray);
	std::vector<std::vector<float>> sampleBufferInit((width*height), initArray);
	std::vector<std::vector<float>> glyphBuffer((width*height), initArray);

	// create distribution buffer to gather light distributions for all positions y,x and directions t
	std::vector<std::vector<std::vector<float>>> distBuffer(dim*steps, glyphBuffer); // initialize #steps 2D-planes w. empty glyphBuffer
	
	// std::vector<std::vector<float>> dist = distBuffer.at(0) - distBuffer.at(0); // Exemplary OPERATOR USE TODO!

	std::vector<float> initGradient(3, 0.0); // construct 3D Gradient
	std::vector<std::vector<float>> deltaBuffer(width*height*steps, initGradient); // initialize #steps 2D-planes w. empty glyphBuffer

	std::vector<std::vector<float>> lightSrcs;
	cout << "before compute glyphs" << endl;
	
	std::vector<std::vector<float>> glyphParameters(dim, std::vector<float>(3, 0.0));
	...
	float meanA = 0.0; // set up mean variables for threshold comparison as STOP criterion..
	float meanMem = 0.0;
	...
	std::vector<float> sample = initArray;
	...
	for (int t = 0; t < steps; t++)
	{
		...
		duration = ((std::clock() - start)*1000.0 / (float)CLOCKS_PER_SEC);
		cout << "timer: " << duration << " ms" << endl;
		cout << "..after propagation, (last) ctr:" << ctr << endl;
		cout << "..trivial ctr:" << tCtr << endl;
	}
	total = ((std::clock() - startTotal)*1000.0 / (float)CLOCKS_PER_SEC);
	...
	std::vector<float> gradient(3, 0.0); // dim3: x,y,theta
	...
	for (int t = 0; t < steps; t++)
		for (int j = 0; j < height; j++)
			for (int i = 0; i < width; i++)
			{
				...
				// X-1D central differences.. VARIANT 2: cell by cell - assumption: uniform spatial sampling w. 1/m --> no normalization
				meanA = 0.0;
				for(int k = 0; k < dim; k++)
				...
				gradient.at(2) = meanA / (2.0*radres);

				deltaBuffer.at(j*width + i + t * dim) = gradient;
			}

	// DELTA (Gradient) COMPUTATION END //
	duration = ((std::clock() - startTotal)*1000.0 / (float)CLOCKS_PER_SEC);
	...
	std::vector<float> scalarNorm(width*height*steps, 0.0); // construct 3D Gradient Norm Vector
	...
	for (int t = 0; t < steps; t++)
		for (int j = 0; j < height; j++)
			for (int i = 0; i < width; i++)
			{
				float res = vectorNorm(deltaBuffer.at(j*width + i + t * dim).begin(), deltaBuffer.at(j*width + i + t * dim).end());
