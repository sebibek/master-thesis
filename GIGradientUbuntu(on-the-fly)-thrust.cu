...
int main(int argc, char* argv[])
{
	...
	thrust::device_vector<double> initArray(steps, 0.0);

	// define dual buffers for propagation
	std::vector<thrust::device_vector<double>> sampleBufferA(dim, initArray);
	std::vector<thrust::device_vector<double>> sampleBufferB(dim, initArray);
	std::vector<thrust::device_vector<double>> sampleBufferInit(dim, initArray);
