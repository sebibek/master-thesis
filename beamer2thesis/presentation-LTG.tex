...
\end{small}
%\bigskip
Symbolic computation of eigensystem:
\begin{itemize}
...
\begin{itemize}
	\item using light propagation volumes (LPV) in a style of Dachsbacher et al. (cf. Fig.)
	\item exchanging energies between neighboring cells: $\Phi_{t} = \int I(\omega)\mathop{d\omega}$\\
...
\begin{enumerate}
	\item Accumulation: evaluate and accumulate the polar intensity profiles
	\item Apply the linear combination (partition) weights
...
\begin{align*}
	T(\omega) = \frac{ab}{\sqrt{a^{2}\sin^{2}(\omega-\varphi)+b^{2}\cos^{2}(\omega-\varphi)}} =\frac{\lambda_1\lambda_2}{\sqrt{\lambda_1^2\sin^2(\omega-\varphi)+\lambda_2^2\cos^2(\omega-\varphi)}}
\end{align*}
{\small w. $\varphi=\mathop{atan2}(\lambda_{1,y},\lambda_{1,x})$ and $\lambda$: singular value, $a$: x-radius ellipse, $b$: y-radius ellipse}
\end{block}
...
n_f &= \frac{\mean{T}\mean{I}}{\widebar{TI}}\hskip 15pt \mathit{w.} \ \mean{f}=\frac{1}{2\pi}\int_0^{2\pi}f(\omega)\mathop{d\omega}
\end{align*}
\end{block}
\begin{itemize}
...
\begin{enumerate}
	...
	\item Accumulation: evaluate product of intensity and transmission profiles and accumulate
...
\begin{itemize}
	...
	\end{align*}
	\item i.e., we gather the total energy in the field and compare it to the last iteration until a state of equilibrium is reached, whereas the flow map takes in a stable state
\end{itemize}
\end{block}
} % END OF FRAME
...
\begin{itemize}
	...
	\item the resulting volume grid is raised by one dimension $d=2+1=3$ for taking direction as additional input argument and computing the Euclidean norm of the gradient
