...
\begin{itemize}
	...
	\begin{itemize}
		\item a) glyphs
		\item b) tensor field lines (TFLs)\\ $\Rightarrow$ streamlines of eigenvector fields
...
\begin{itemize}
	...
	\item consisting of $3$ stress vectors arranged in row-major order
\end{itemize}
\end{column}
\begin{column}{.5\textwidth}
\begin{figure}[t]
...
\begin{itemize}
	...
	\item FTLE field largest where path lines diverge
	\item ridges in FTLE field represent Lagrangian coherent structures: separating structures
\end{itemize}
\end{small}
...
\begin{small}
\begin{itemize}
	\item eigensystems/SVS are computed at samples of grid
	\bigskip
...
\begin{itemize}
	...
	\item trajectories can cross each other in degenerate points\\
...
%based propagation through diffusion tensor fields", 1999}: introduce artificial inertia to TFLs to increase stability
	...
	\item TFL-FTLE\footnote[frame]{Hlawatsch et al. ''Coherent structures of
...
\begin{columns}
\begin{column}{.6\textwidth}
To interpret singular value systems of tensors as conductivity property for light transport, we will need a propagation scheme:\bigskip
\begin{itemize}
...
\begin{figure}[!t]
  \centering
 {
  ...
  \caption*{Impulse response: placement of cosine lobes}
  \label{scheme}
\end{figure}
\begin{align*}
...
\begin{itemize}
	...
	$\Rightarrow$ switching source and target grid
	...
	\end{align*}
	\end{block}
	$\Rightarrow$ a state of equilibrium is reached, where the flow map $\mathcal{C}$ takes in a stable state\\
...
\end{figure}\bigskip
\begin{itemize}
	\item Inverse test field reveals strong ridges for diverging light distributions
	\item top ridge missing for converging transmission profiles
\end{itemize}
} % END OF FRAME
...
\begin{minipage}{0.21\textwidth}
  ...
  \begin{minipage}{0.21\textwidth}
  \centering
  \includegraphics[height=\textwidth]{brain_ftle_surf_noisereduction.PNG} {\scriptsize b) T-FTLE}
     \label{a)}
  \end{minipage}\hskip 15pt
  \begin{minipage}{0.21\textwidth}
    \centering
...
\begin{itemize}
	...
	\item FTLE response on structure obtained similar to Hlawatsch et al.
\end{itemize}
} % END OF FRAME
...
\begin{itemize}
	...
	\item red regions exhibit most rapid changes in direction and/or divergence of TFLs
