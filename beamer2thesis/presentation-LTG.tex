...
\centering
  ...
  \begin{minipage}{0.3\textwidth}
    ...
    \label{b)}
  \end{minipage}
\caption*{Random field: a) field, b) tensor magnitude, c) ours(LTG)}
\label{test_fields2}
\end{figure}
Applications:
\begin{itemize}
...
\begin{itemize}
	...
	\bigskip
	\item light propagation volumes: C. Dachsbacher et al., ''Cascaded light propagation volumes for real-time indirect illumination", 2010
\end{itemize}
...
\begin{figure}[t]
\centering
\includegraphics[height=0.34\textwidth]{diverge.png}~a)
...
\begin{itemize}
	\item using light propagation volumes (LPV) in a style of Dachsbacher et al. (cf. Fig.)\bigskip
...
n_f &= \frac{\mean{T}\cdot\mean{I}}{\widebar{T\cdot I}}\hskip 15pt \mathit{w.} \ \mean{f}=\frac{1}{2\pi}\int_0^{2\pi}f(\omega)\mathop{d\omega}
\end{align*}
\end{block}
\begin{itemize}
...
\centering
  ...
  \centering
    ...
    \label{b)}
  \end{minipage}
  \caption*{Heart test field: a) Glyphs, b) H.'s approach, c) LTG (rescale)}
\label{heart-ftle}
\end{figure}\bigskip
\begin{itemize}
...
\centering
    ...
    \caption*{Random test field : a) Tensor magnitude, b) Local normalization, c) Tensor mag. d) LTG}
\label{bow_contour}
\end{figure}
%\begin{figure}[!t]
%\centering
