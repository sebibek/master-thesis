...
\begin{align*}
	...
	\Phi_{t} &= \varepsilon_k\int_a^b I(\omega)\mathop{d\omega}
\end{align*}
{\small with $\Phi_{t}$: radiant flux, $I(\omega)$: radiant intensity}
\end{block}
\end{column}
...
\begin{itemize}
	...
	\end{align*}
	\end{block}
%	\item a state of equilibrium is reached, where the flow map takes in a stable state
%	\item i.e., a stop sequence is initiated when above criterion is met 
...
\end{block}
\begin{itemize}
	\item previous examples prove that the conductivity property (transmission) of the tensor field can effect a separation of the flux for diverging pathlines
	...
	\item FTLE field is yielded from accumulating the total radiant flux in the difference scalar field for varying location and direction 
\end{itemize}
} % END OF FRAME
...
\centering
  \begin{minipage}{0.6\textwidth}
  \centering
	\includegraphics[height=0.65\textwidth]{skizze/addins/grids_two_edit.pdf}
	\caption*{Light Transport Gradient - Neighborhood}
	\end{minipage}
\end{figure}
...
\end{minipage}
\begin{minipage}{.49\textwidth}
\includegraphics[height=\textwidth]{iso_ftle.png} \caption*{b)~LTG}
\end{minipage}
...
\begin{minipage}{.49\textwidth}
\centering
\includegraphics[height=\textwidth]{iso_isovolume.png} \caption*{b)~LTG}
\end{minipage}
