...
\begin{itemize}
	\item Each position in space is mapped a scalar value: $\mathop{f(\mathbf{x})\colon} \mathbf{x}\mapsto f\mathbf{(x)}$
	\bigskip
...
\begin{column}{.5\textwidth}
\begin{figure}[t]
\includegraphics[width=0.4\textwidth]{vector_field_streamline.pdf} a)
\includegraphics[width=0.4\textwidth]{vector_field2_streamline.pdf} b)
\caption{Vector fields: a) $\begin{pmatrix}
x\\y
\end{pmatrix}$, b) $\begin{pmatrix}
-y\\x
...
\begin{small}
Definition: $\mathop{f\colon} \mathcal{V}\to \mathcal{V}$, \hskip 10pt $f(\mathbf{x})=\lambda\mathbf{x}$,\hskip 10pt w. $\mathcal{V}\in\mathcal{R}^n$\vskip 5pt
...
\begin{itemize}
	...
	\bigskip
	\item glyphs are used to represent the eigensystem
	\bigskip
