...
For the rest of this work, we will give a quick overview on the structure and outline. Next, in Chap.~\ref{chap:grundlagen}, we will capture important fundamentals like tensors, polar coordinates, PCA in the context of singular value decomposition (generalization of eigenvalue decomposition), FTLE and GT (ground truth) datasets. Also we will discuss the most relevant related work in the domain of tensor field visualization and also in the domain of global illumination (indirect illumination rendering). Subsequently, in Chap.~\ref{chap:meth}, we will propose the main contributions and methods of this work and document the results and evaluation in Chap.~\ref{chap:eval}. At last, a compact summary, conclusion and outlook will be given in Chap.~\ref{chap:concl}.\\
...
\centering
  ...
  \begin{minipage}{0.4\textwidth}
    ...
    \label{b)}
  \end{minipage}
\caption{``drain'' test field: a) glyphs, b) tensor field lines for a)}
\label{drain}
\end{figure}
...
\centering
  ...
  \begin{minipage}{0.4\textwidth}
   \centering
    ...
    \label{b)}
  \end{minipage}
\caption{``rings'' test field: a) glyphs, b) tensor field lines for a)}
\label{rings}
\end{figure}
...
\centering
  ...
  \begin{minipage}{0.4\textwidth}
    ...
    \label{b)}
  \end{minipage}
\caption{``spiral''-test field: a) glyphs, b) tensor field lines for a)}
\label{spiral}
\end{figure}
...
\centering
  ...
  \begin{minipage}{0.4\textwidth}
    ...
    \label{b)}
  \end{minipage}
\caption{``inverse'' test field: a) glyphs, b) tensor field lines for a)}
\label{inverse}
\end{figure}
...
\centering
  \begin{minipage}{0.4\textwidth}
    ...
    \label{a)}
	a) light source in center
  \end{minipage}
  \begin{minipage}{0.4\textwidth}
    \centering
    \includegraphics[width=0.9\textwidth]{img/iso_sat.png}
    \label{b)}
	b) light source in center (rescaled)
  \end{minipage}
...
\centering
  ...
  \centering
	...
	c)
  \end{minipage}
  \caption{``drain'' test field: a) TFLs and glyphs, b) LTG($90^\circ$) for a), c) gradient for b)}
\label{ftle_base}
\end{figure}
...
For the ``inverse'' test field, we can observe a 3 part ``rotor'' indicating the ridges which are in parallel up to orthogonal to the current light source (slice) direction. This rotor can be obtained by applying a contour filter, but it can also be visualized by applying a simple thresholding on the surface representation of the volume, as depicted in Fig.~\ref{inverse_volume}. Subsequently, we will analyze the \enquote{rings} test field to show its performance on circular ridges.
...
\centering
  ...
  \end{minipage}
   \begin{minipage}{0.3\textwidth}
	...
	c)
  \end{minipage}
    \caption{LTG volumes: ``rings'' test field: a) Contour, b) IsoVolume, c) Volume}
\label{rings_contour}
\end{figure}
...
When we consider the LTG method, we already stated its runtime behavior ($\mathcal{O}(\mathop{width^5}\mathop{steps^2})$), which makes parallelization libraries and hardware to compute the results in reasonable time. First, we tried to implement a GPU-based parallelization for the propagation scheme with nVidia's Thrust library, which turned out not to work as expected, since the relative runtime gain was too low to make a big difference and we struggled to find a suitable implementation mapping the propagation process to atomic OPs (simple vector operations), which are the only operations allowed for GPU-parallelization. At this point, we choose to use OpenMP as we figured it is a much more suitable architecture employed here, which allows to perform the computation on a multiprocessor architecture provided by the Heidelberg University allowing to compute $64$ hardware threads at maximum. As we need fat fed, instruction-rich threads to have a high performance gain, we choose to parallelize only the outer loop of the LTG method (propagating final light distributions for each light source direction). Remark, that each thread requires its own memory, as an independent memory block is needed to perform the propagation in an individual propagator object of class propagator, in order to perform the computation in parallel.
...
At first, we needed to generate convenient test cases, which we design using Mathematicas VectorPlot, used to visualize vector fields. To output a predefined test case, we need to define a little code, for which we used python scripts which generate VTK appended or ASCII format files. To implement the method we used C++'s STL for every dynamic memory (buffer), SFML to plot polar profiles, Eigen to perform matrix decompositions and VTK to in- and output VTK format files, which can in turn be visualized and explored interactively with the visualization framework ParaView. The user functions defined in the configuration file are read-in by a string parser (muParser Library), which is used just once for sampling the polar profile and storing the precomputed result. This causes a convenient input mechanism for stimuli, in case they should be predefined.
...
\label{sec:att}
It can be proven that the approach follows propagation attenuation principles and follows the inverse (square in $3D$) law ($\sim\frac{1}{r}$) as observed in the graph in Fig.~\ref{att}. For that, we plot the relative intensity $i_{rel}(r_{rel})$ (in relation to the intensity at position (radius) $1$ to account for the inverse square law, which holds for relative distances. We evaluate the relative intensity by forming a ratio $\frac{I(r)}{I_1}$, that relates the step intensity to the source intensity at distance $1$.
...
\label{gyre-ftle}
\end{figure}
In Fig.~\ref{gyre-ftle}, we can observe the method segmenting ridges in local ROIs exhibiting diverging tensor field lines w.r.t. the $x$-axis ($0^\circ$). Note that even regions in antiparallel directions ridges are detected to (literally) some degree: Remark the ridges being contiguos lines at the level of the gyre centers where converging turns into diverging. Again, it is noticeable, that the result of Hlawatsch correspond to our result to a certain degree. It is noticeable, that Hlawatsch's approach does not consider the initial direction and hence will detect ridges also in antiparallel directions (gradient of the flow map). This yields symmetric ridges w.r.t. central axes, whereas we detect ridges only up to orthogonally to the initial light source direction as visible in Fig.~\ref{gyre-ftle}b)\& c), respectively. In the center location of each gyre, we interpolate the sorrounding ring with an isotropic cell in the middle, leading to chaotic trajectories leaving the gyre and thus leading to an increased gradient of the flow map for Hlawatsch's result. This causes the FTLE field to respond in the center of the gyres in Fig.~\ref{gyre-ftle}c). We interpret this behavior within the scope of interpolated trajectories, which are in general more likely to escape the gyre, since their angle is interpolated (by PCA), which is susceptive to rapid changes and hence leads to unstable behavior. What Hlawatsch et al. detect, could be called local ridges (in terms rather short), what our method detects could rather be called global ridges (in terms rather long). This implies, that we obtained reasonable results generated by our method, but the classical tensor field line FTLE is susceptive to instability concerning its directive abilities and does not do things by halves (like we do with our partitioned light distributions), which causes sharper ridges in general. Also remember, that the resulting ridges are not to be concerned sharp ones, since we measure the separation of whole light distributions, which should be considered a continous response or measure. We again depict volume representations for this example:
\begin{figure}[!t]
\centering
  \begin{minipage}{0.3\textwidth}
...
The volumes depicted in Fig.~\ref{heart-ftle} now show variation in $z$-direction, as they are now visualizing pure FTLE fields with no weighting of the tensor magnitude incorporated.
...
\label{heart-ftle}
\end{figure}
Fig.~\ref{heart-ftle} depicts the volumes for the heart test field, whereas we can observe a 3D ridge running in spiral along the $z$-direction. The FTLE responses are shifted w.r.t. varying directions along the $z$-axis.

We also introduced random numbers in to the our tensor field generation process. Therefore, we define a uniform grid with low resolution ($9\times9$) and interpolate bilinearly to obtain a tensor field with higher resolution exhibiting continous changes in anisotropy and tensor magnitude.
...
\centering
  ...
  \includegraphics[width=\textwidth]{img_new/random-global.png}
    \caption*{Global normalization}
    \label{a)}
  \end{minipage}
  \begin{minipage}{0.4\textwidth}
   \centering
    \includegraphics[width=\textwidth]{img_new/random-local.PNG}
     \caption*{Local normalization}
    \label{b)}
  \end{minipage}
