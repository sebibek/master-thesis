...
start all over (after reinitialization). Another reason is it yields a reasonable source-target structure. We introduce a new operator called \textit{propagate}, which we will fill with meaning in sect. \ref{sec:trans} in form of a precise mathematical formulation. Each Grid or Buffer is basically a set of cells defined as $\mathcal{C} = \{\mathbf{c}_1, \mathbf{c}_2, ..., \mathbf{c}_n\}$  The dual buffer result $\mathcal{C'}$ is denoted symbolically as follows for all cells w. a mean intensity greater than zero (which do not contain Null samples):
\begin{align}
	\mathcal{C'} = \mathop{propagate}\{c_i\in \mathcal{C} | \ |\mean{I}_i| > 0\} \hskip 5pt {w.} \ i = \{1..dim\}
\end{align}
Note that, $dim=width\cdot height$, which equals the tensor field resolution in this case.
