...
There has been extensive recent work on symmetric tensors but in comparison little on asymmetric ones. Since we aren't able to cover the whole scope in this work, we will focus on the most relevant works on tensor field visualization. We will talk about the publications from Hlawatsch et al. \cite{hlawatsch} and Zirr et al. \cite{zirr} in particular, since they constitute thematically close related works.\vskip 5pt
...
\label{sec:scheme}
As already discussed, we will motivate a light transport propagation scheme designed to efficiently propagate initial intensity profiles stored in polar coordinates given as symbolic impulses (stimuli) by the user defined in the configuration file. This polar profiles are first sampled into a vector with elements ranging over the interval $[0,2\pi]$. Polar profiles for different light sources are shown in Fig. \ref{polar}.
\begin{figure}[!t]
  \centering
 {
    \includegraphics[width=0.8\textwidth]
...
\label{sec:ltg}
Now, that we are able to propagate intensity in grid, directed by the eigenvectors of the tensor field, we aim to do something more elaborate to segment ROIs (regions of interest) characterised by divergence/convergence of tensor field lines and drastic changes in anisotropy. We consider the following light distributions and their difference image to have an idea, for which kind or types of cases we obtain inherently different light distributions. 
...
\centering
  \begin{minipage}{0.3\textwidth}
    ...
    \label{a)}
    \caption*{a) final light distribution for left offset}
  \end{minipage} $-$
  \begin{minipage}{0.3\textwidth}
    \includegraphics[width=\textwidth]{img/ftle_right.png}
    \label{b)}
    \caption*{b) final light distribution for right offset}
  \end{minipage} $=$
   \begin{minipage}{0.3\textwidth}
