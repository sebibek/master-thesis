...
\State $\mathit{index} \gets (j\cdot\mathit{width}+i)\cdot steps$ \Comment{compute $1D$ (cell) index}
...
\State $n_c \gets \frac{\mu_{T_c}\mu_{I_c}}{\mu_{T_cI_c}}$\Comment{Comp. normalization factor for cur. cell}
 \For{each direction $k$ in $[0,7]$}\Comment{For all neighbors, do..}
	...
	\State $\mathit{bufferB(index}\gets \mathit{energy}\cdot cos_k(\omega)+\mathit{bufferB(index}\cdot\mathit{steps)}$\\
...
\section{Light transport gradient (LTG)}
The light transport gradient approach, which is closely related to the FTLE approach in vector fields, is evaluated for some more synthetic and real datasets in this section. We first evaluate a classic textbook example the \enquote{Double Gyre}, which is a quadruple gyre in our case for formatting reasons, yet yielding the same conceptual results. We compare the glyph representation to the LTG magnitude for a single slice ($0^\circ$) again:
\begin{figure}[!t]
\centering
  \begin{minipage}{0.4\textwidth}
