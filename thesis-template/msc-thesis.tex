...
\label{star_ftle}
\end{figure}
When we look at the example depicted in Fig. \ref{star_ftle}, we can see the FTLE approach LTG responding most in the center of the field, which is the knot point of tensor field lines diverging in any direction. The volume illustrated on the right even shows a spiked structure for the volume representing all directions at once. The approach detects more ridges in diagonal directions stochastically as we can infer from the Gaussian-shaped wings of the volume, evenly spaced in $90^\circ$ offsets. But wouldn't we expect a cylinder shape for a symmetric testcase? These can be interpreted nicely, since the diagonal direction serves a factor $\times\sqrt{2}$ longer mean straight line distance (SLD), leading to $\times\sqrt{2}$ more visited cells on average. The energy distribution will be more complete (concerning the occupation of energy in the grid) and hence we will obtain larger differences for our light transport gradient for the diagonal direction, which should be seen gradually and continously as we observe Gaussian-plates in the volume instead of sharp, contrasted ridges. A possible correction would be, to normalize each slice with a factor $\frac{1}{1+(\sqrt{2}-1)\lvert\cos2\omega-\frac{\pi}{4}\rvert}$ to get a uniform response over all directions. This shows a limitation of the approach, as diagonal ridges are detected predominantly over ridges in axis directions in case both existent, as this test example states.
...
\toprule
\enquote{drain} & $101\times101\times64=652864$ & $179878$ & $4\times652864\times 8\mathop{byte} +(101\times101 + 19\times64)\times8\mathop{byte}\approx20.99\mathop{MB}$\\ 
\hline 
 &  &  & \\ 
\hline 
 &  &  & \\ 
