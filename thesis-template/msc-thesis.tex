...
\centering
  ...
  \end{minipage}
  \begin{minipage}[!t]{0.4\textwidth}
    \includegraphics[width=\textwidth]{img/rings_ftle.png}
    \label{b)}
    \caption*{b) LTG($0^\circ$) for a)}
  \end{minipage}
...
\label{gyre_volume}
\end{figure}
Regarding Fig. \ref{gyre_volume}, we find that the approach detects the boundaries of concavities (lat.: Gyri) in the brain, forming the lobus cerebri (commonly known a lobes). The invented method could thus ,e.g. , aid to compute $3D$ models of a subject in medical sciences. It emphasizes regions with much flow-rate (throughput), because in this areas the anisotropy and/or tensor magnitude is increased, preventing the intensities from being absorbed, hence yielding to a high FTLE-response. Note that we do not show the volume representation in this case, since all directions (slices) look approximately the same because of the tensor field only introduces changes in tensor magnitude not in anisotropy as inferred from the glyph representation.
...
\centering
  ...
  \end{minipage}
  \begin{minipage}{0.4\textwidth}
    \includegraphics[height=\textwidth]{img/heart_org.PNG}
     \caption*{LTG}
    \label{b)}
  \end{minipage}
