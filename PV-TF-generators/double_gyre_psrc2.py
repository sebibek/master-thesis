...
def double_gyre_vectorized( x, y, t):
	...
	dx = -np.pi * A * np.sin(np.pi * f) * np.cos(np.pi * y)
	dy = np.pi * A * np.cos(np.pi * f) * np.sin(np.pi * y) * df
	dt = np.full_like(t, 1.0)

	return np.dstack([dx, dy, dt])
...
img.PointData.append(v.reshape((-1, 3), order='F'), 'v')
