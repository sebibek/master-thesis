...
with open('temp.txt', 'wb') as f:
    for j in range(length): # rows
        for i in range(length): # cols
            y = round((length-1)*m.cos(2*i/(length-1))-(length-1)/2) # use inverted y coordinates (mirroring at x-axis in array reference frame)
            x = round(j - (length-1)/2) # --> center around midpoint
